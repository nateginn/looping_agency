...
allowed_actions:
  ...
  - type: internal-link-addition
    ...
    min_sample_size: 150
# 2026-08-16: back to propose-only. Standing decision 6 ("automation stays off until
# 'did it work?' is trustworthy") and SEO_PROGRAM.md Part 3A priority 0 both said this,
# while these two lines still said tier1-enabled/true - three records, two positions.
# `approval_mode` is the switch that actually closes both doors: apply.py:216-222 reads it
# fresh from this file at apply time and refuses every Tier-1 apply regardless of whether a
# human `approved` or Codex review `approved-for-implementation` reached it.
# `auto_implementation_enabled: false` is defence in depth - on its own it only gates the
# codex-review path (apply.py:233), leaving the human-approval route to apply.py wide open.
# Restore both to tier1-enabled/true on an explicit, recorded go/no-go - not by default, and
# not before the backlog's "did it work?" item lands (there is still no 'worse' outcome).
...
- **2026-08-03: Phase 7 activated for `art`** (`PLAN-PHASE7-CODEX-REVIEW.md`, `HANDOFF.md`). `approval_mode: tier1-enabled` and `auto_implementation_enabled: true` were both set; `title-tag-rewrite`/`meta-description-rewrite` have `manual_approval_only: false`, so those two action types could reach a **local, unpushed git commit** via `/codex-seo-review` once two independent rounds of Codex adversarial review agree — no push, merge, or deploy is possible either way (Tier 2, human-only, unconditionally). `internal-link-addition` stays `manual_approval_only: true` deliberately — no code mutates a live Django template to insert a link, so it can never auto-implement regardless of this flag; it always lands at human review. `apply.py` re-reads this file fresh at commit time, so flipping any of these three gates back off genuinely disables the path for anything not already committed.
- **2026-08-16: deactivated — back to `propose-only` + `auto_implementation_enabled: false`** (see the comment above those keys, and `CURRENT-WORK.md`). Nothing was ever auto-implemented under the 8/03 activation: zero proposals reached `applied`, and `pending/` was empty at the time of the change, so no already-created proposal carries a stale cached `manual_approval_only: false` into the frozen state. Proposals are still *generated* — `propose-only` blocks application, not drafting — so the loop keeps producing candidates for human review via `/review-pending`. The `manual_approval_only` values on the three actions are left as they are; they only matter once `approval_mode` returns to `tier1-enabled`.
