...
- `gbp` — **scaffolded, not enabled.** `projects/art/loops/gbp/spec.md` exists and validates (Phase 1/2 of the Google Business Profile Posts loop design — see that file's own Notes for what's built vs. not) but is deliberately absent from `loops_enabled` above and unregistered with Windows Task Scheduler. It collects local-rank/indexation observability only; no proposal selector exists yet, so it generates zero proposals, and no publish path exists at all — nothing in this workspace can create or edit a live GBP Post today. Do not add `gbp` to `loops_enabled` or schedule it until at minimum Phase 3 (selector) exists; publishing (Phase 6) additionally requires the Phase 0 prerequisites in `spec.md` (a verified place_id/cid per location, and separate Google API access for posting vs. measurement), which only Nate can complete.
