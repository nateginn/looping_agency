...
2. Run:
   ```
   node tools/run-loop.mjs <project> <loop>
   ```
3. Read the JSON result:
   ...
   - `status: "invalid-spec"` → report the validation errors from `runs/<run-id>/validation-failure.json`; the loop did not run. Point at `node tools/spec-validate.mjs <spec path>` for details.
...
4. Never call `tools/apply.mjs` from this skill. Applying a proposal is always a separate, explicit `/review-pending` action gated on human approval.
