...
2. Run:
   ```
   ./.venv/Scripts/python.exe tools/run_loop.py <project> <loop>
   ```
3. Read the JSON result:
   ...
   - `status: "invalid-spec"` → report the validation errors from `runs/<run-id>/validation-failure.json`; the loop did not run. Point at `./.venv/Scripts/python.exe tools/spec_validate.py <spec path>` for details.
